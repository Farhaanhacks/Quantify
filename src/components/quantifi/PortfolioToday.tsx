...
// The two cards that make up the snapshot. Pulled out of the component so the
// sample preview renders the exact same layout a real portfolio gets — no
// separate "marketing" version that can drift from the product.
function SnapshotBody({
  ...
  const byCurrency: Record<string, { value: number; dayChange: number }> = {};
  for (const l of lines) {
    const b = byCurrency[l.currency] ?? { value: 0, dayChange: 0 };
    b.value += l.value;
    b.dayChange += l.dayChange;
    byCurrency[l.currency] = b;
  }

  const movers = [...lines].sort((a, b) => b.dayPct - a.dayPct);
...
// Same two-card grid, shimmering — so the section has its real shape from the
// first paint instead of collapsing to a one-line "Loading…".
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
      <GlassCard className="flex flex-col justify-center p-6">
        ...
        <Skeleton className="mt-4 h-2.5 w-48" />
      </GlassCard>
      <SkeletonTable
        rows={4}
        cols={["1.4fr", "0.8fr", "1fr"]}
        headers={["Holding", "Today", "Day P/L"]}
      />
    </div>
  );
}

const SAMPLE_LINES: Line[] = SAMPLE_HOLDINGS.map((h) => ({
  ticker: h.ticker,
  name: h.name,
  currency: h.currency,
  value: h.shares * h.price,
  // Day P/L implied by the sample's day-change percentage.
  dayChange: h.shares * h.price * (h.dayPct / (100 + h.dayPct)),
  dayPct: h.dayPct,
}));
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      {!ready || loading ? (
        ...
        // No holdings yet — show the module working on an example book rather
        // than an empty card, so a first-time visitor can see what they'd get.
        <SamplePreview
          className="mt-6"
          note="This is an example portfolio. Add your holdings and this snapshot runs on your own positions, live."
          cta="Build your portfolio →"
          href="/portfolio"
        >
          <SnapshotBody lines={SAMPLE_LINES} usdinr={SAMPLE_USDINR} />
        </SamplePreview>
      ) : (
        <div className="mt-6">
          <SnapshotBody lines={lines} usdinr={usdinr} />
        </div>
      )}
    </section>
  );
