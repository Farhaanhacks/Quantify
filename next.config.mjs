...
// ───────────────────────────────────────────────────────────────────────────
// This is scoped to the exact third parties Quantifi's *browser* loads:
...
//
// Google fonts are self-hosted at build time by next/font, so 'self' covers
// them — no fonts.googleapis / fonts.gstatic entries required.
//
...
// To roll CSP out safely on a live app, flip this to `true` once. It emits
// Content-Security-Policy-Report-Only, which logs violations to the browser
// console WITHOUT blocking anything — watch for a few days, then set back to
// false to enforce. (Default: false = enforcing, since the policy above is
// already scoped to your real dependencies.)
...
const securityHeaders = [
  ...
  // you open / that open you (blocks cross-window "tab-nabbing" attacks and is
  // the flag security scanners look for). We use `same-origin-allow-popups`
  // rather than the stricter `same-origin` so Razorpay's payment / 3-D-Secure
  // popups keep their window.opener link and can post the result back. Google
  // sign-in is a full-page redirect here, so it's unaffected either way.
  ...
  // Stop MIME-type sniffing (a classic XSS/content-confusion vector).
  ...
  // Force HTTPS for 2 years. Safe on Vercel (HTTPS-only). If you serve any
  // subdomain over plain HTTP, drop `includeSubDomains`. Add `; preload` only
  // once you're certain and want to submit to the HSTS preload list.
  {
...
// give you full cross-origin isolation but immediately break TradingView,
// Razorpay and remote images, which don't send the CORP/CORS headers it demands.
